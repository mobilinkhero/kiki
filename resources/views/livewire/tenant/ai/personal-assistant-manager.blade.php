...
<div>
    ...
    <div class="sm:flex sm:items-center sm:justify-between">
        ...
        <div class="mt-4 sm:mt-0 flex gap-3">
            <!-- Dashboard Link -->
            <a href="{{ route('tenant.ai.dashboard', ['subdomain' => request()->route('subdomain')]) }}" 
                class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors">
                📊 Performance Dashboard
            </a>
            
            @if(!$assistant)
                <button wire:click="createAssistant" type="button"
        ...
        @foreach($assistants as $assistant)
            <div class="mt-6 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 max-w-md">
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <div class="flex items-center space-x-2 mt-1">
                                    <span
                                        class="text-xs font-medium {{ $assistant->is_active ? 'text-green-600' : 'text-gray-500' }}">
                                        {{ $assistant->is_active ? 'Active' : 'Inactive' }}
                                    </span>
                                    <span
                                        class="text-xs text-gray-500">{{ $availableModels[$assistant->model] ?? 'gpt-4o-mini' }}</span>
                                </div>
                            </div>
                            <div class="flex items-center space-x-2">
                                <span class="text-xs text-gray-500">{{ $assistant->is_active ? 'Active' : 'Inactive' }}</span>
                                <button wire:click="toggleAssistant({{ $assistant->id }})" type="button"
                                    ...
                                    <span
                                        class="inline-block h-4 w-4 transform rounded-full bg-white shadow-sm transition-transform duration-200 ease-in-out {{ $assistant->is_active ? 'translate-x-6' : 'translate-x-1' }}"></span>
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex items-center space-x-2 mb-6">
                    ...
                    <span class="text-sm text-gray-600">{{ $assistant->hasUploadedFiles() ? $assistant->getFileCount() : 1 }}
                        document</span>
                </div>
                ...
                <div class="space-y-4 border-t border-gray-200 dark:border-gray-700 pt-4">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <span
                                class="text-xs font-medium {{ $allSynced ? 'text-green-600 bg-green-50' : 'text-yellow-600 bg-yellow-50' }} px-2 py-1 rounded">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-2">
                                <div class="w-2 h-2 {{ $assistant->is_active ? 'bg-green-400' : 'bg-gray-400' }} rounded-full">
                                </div>
                                <span class="text-sm text-gray-700 dark:text-gray-300">AI Assistant</span>
                            </div>
                            <span
                                class="text-xs font-medium {{ $assistant->is_active ? 'text-green-600 bg-green-50' : 'text-gray-600 bg-gray-100' }} px-2 py-1 rounded">
                                {{ $assistant->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-2">
                                <div
                                    class="w-2 h-2 {{ $assistant->hasUploadedFiles() ? 'bg-green-400' : 'bg-gray-400' }} rounded-full">
                                </div>
                                <span class="text-sm text-gray-700 dark:text-gray-300">Knowledge Base</span>
                            </div>
                            <span
                                class="text-xs font-medium {{ $assistant->hasUploadedFiles() ? 'text-green-600 bg-green-50' : 'text-gray-600 bg-gray-100' }} px-2 py-1 rounded">
                    ...
                    <div class="border-t border-gray-200 dark:border-gray-700 pt-3">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <span
                                class="text-xs text-gray-500">{{ $assistant->hasUploadedFiles() ? $assistant->getFileCount() : 0 }}
                                total</span>
                        </div>
                        @php
                            $syncedCount = 0;
                        ...
                        @if($syncedCount > 0)
                            <div class="bg-green-50 dark:bg-green-900/20 rounded px-3 py-2 mb-1">
                                <span class="text-xs font-medium text-green-700 dark:text-green-400">{{ $syncedCount }}
                                    Synced</span>
                            </div>
                        @endif
                        ...
                        @if($pendingCount > 0)
                            <div class="bg-yellow-50 dark:bg-yellow-900/20 rounded px-3 py-2">
                                <span class="text-xs font-medium text-yellow-700 dark:text-yellow-400">{{ $pendingCount }}
                                    Pending</span>
                            </div>
                    ...
                    <div class="flex items-center space-x-3 mt-6">
                        <button wire:click="openChat({{ $assistant->id }})"
                            class="flex-1 bg-blue-600 text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors flex items-center justify-center shadow-sm h-10">
                            <x-heroicon-s-chat-bubble-left-right class="w-4 h-4 mr-2" />
                            Chat
                        </button>
                        <button wire:click="syncAssistant({{ $assistant->id }})" wire:loading.attr="disabled"
                            class="flex-1 bg-purple-600 text-white px-3 py-2.5 rounded-lg text-sm font-medium hover:bg-purple-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm h-10 min-w-0">
                            <span wire:loading.remove wire:target="syncAssistant({{ $assistant->id }})"
                                class="flex items-center justify-center whitespace-nowrap">
                                Sync Now
                            </span>
                            <span wire:loading wire:target="syncAssistant({{ $assistant->id }})"
                                class="flex items-center justify-center whitespace-nowrap">
                                <svg class="animate-spin h-4 w-4 mr-2 flex-shrink-0" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <button wire:click="openDetails({{ $assistant->id }})"
                            class="flex-1 bg-gray-700 dark:bg-gray-600 text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-gray-800 dark:hover:bg-gray-700 transition-colors flex items-center justify-center shadow-sm h-10"
                            title="View Details">
                            <x-heroicon-s-information-circle class="w-4 h-4 mr-2" />
                            Details
                        </button>
                        <button wire:click="editSpecificAssistant({{ $assistant->id }})"
                            class="w-10 h-10 p-0 bg-gray-100 dark:bg-gray-700 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors flex items-center justify-center shadow-sm"
                            title="Edit Assistant">
                            <x-heroicon-s-cog-6-tooth class="w-5 h-5 text-gray-600 dark:text-gray-400" />
                        </button>
                        <button wire:click="deleteSpecificAssistant({{ $assistant->id }})" wire:confirm="Delete assistant?"
                            class="w-10 h-10 p-0 bg-red-100 dark:bg-red-900/20 rounded-full hover:bg-red-200 dark:hover:bg-red-900/40 transition-colors flex items-center justify-center shadow-sm"
    ...
    @elseif(!$showCreateForm)
                ...
                <div class="border-t border-gray-200 dark:border-gray-700 px-4 py-5 sm:px-6">
                    <form wire:submit.prevent="saveAssistant" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            <div>
                                <label for="name"
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300">Assistant Name
                                    *</label>
                                <input wire:model="name" type="text" id="name"
                                    ...
                                    placeholder="e.g., SmartFlow AI">
                                @error('name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>

                            <div>
                                <label for="model" class="block text-sm font-medium text-gray-700 dark:text-gray-300">AI
                                    Model *</label>
                        ...
                        <div>
                            ...
                            <textarea wire:model="description" id="description" rows="2"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm"
                                placeholder="Brief description of what this assistant helps with"></textarea>
                                ...
                                @foreach($useCaseOptions as $value => $label)
                                    <label class="relative flex items-start">
                                        <div class="flex items-center h-5">
                                            <input wire:model="use_case_tags" type="checkbox" value="{{ $value }}"
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            <div>
                                ...
                                <input wire:model.live="temperature" type="range" id="temperature" min="0" max="2"
                                    step="0.1" class="mt-1 block w-full">
                            ...
                            <div>
                                <label for="max_tokens"
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300">Max Response
                                    Length</label>
                                <select wire:model="max_tokens" id="max_tokens"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm">
                        ...
                        <div>
                            <div class="flex items-center justify-between">
                                <label class="flex items-center space-x-3 cursor-pointer">
                                    <input type="checkbox" wire:model="allow_buttons"
                                    ...
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Allow Interactive
                                        Buttons</span>
                                </label>
                            </div>
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                If enabled, the assistant can include interactive buttons (e.g., choices or actions) in its
                                responses.
                                Note: Responses with buttons are limited to ~800 characters.
                            </p>
                        </div>
                        ...
                        <div>
                            <label for="system_instructions"
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300">System Instructions
                                *</label>
                            <textarea wire:model="system_instructions" id="system_instructions" rows="6"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm"
                                placeholder="Define how the assistant should behave, its role, and guidelines..."></textarea>
                            <p class="mt-1 text-xs text-gray-500">These instructions guide how your AI assistant responds to
                                queries.</p>
                        ...
                        <!-- File Upload -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Upload Files for AI
                                Analysis</label>
                            ...
                            @if($editingAssistantId && $assistant && $assistant->hasUploadedFiles())
                                <div class="mt-2 mb-4 bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                                    <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Currently Uploaded
                                        Files</h4>
                                    <div class="space-y-2">
                                        @foreach($assistant->getFilesWithStatus() as $file)
                                            <div class="flex items-center justify-between bg-white dark:bg-gray-800 rounded p-2">
                                                <div class="flex items-center space-x-2">
                                                    ...
                                                    <span class="text-xs text-gray-500">({{ number_format($file['size'] ?? 0) }}
                                                        bytes)</span>
                                                </div>
                                                <button type="button" wire:click="removeFile('{{ $file['original_name'] }}')"
                                ...
                                <div class="space-y-1 text-center">
                                    ...
                                    <div class="flex text-sm text-gray-600">
                                        <label for="file-upload"
                                            ...
                                            <input wire:model="files" id="file-upload" name="file-upload" type="file"
                                                class="sr-only" multiple accept=".txt,.md,.csv,.json">
                                        </label>
                        ...
                        <div
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    <h3 class="text-sm font-medium text-blue-800 dark:text-blue-200">Guidelines for Best
                                        Results</h3>
        ...
        @if($showChatModal && $chattingAssistantId)
            ...
            <div class="fixed inset-0 z-50 overflow-y-auto">
                <div class="flex min-h-full items-center justify-center p-4">
                    <div class="relative transform overflow-hidden rounded-xl bg-white dark:bg-gray-800 text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-3xl"
                        wire:click.stop>
                        @php
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center space-x-2">
                                    ...
                                    <button wire:click="clearChat"
                                        class="px-3 py-1.5 text-sm bg-red-500 text-white rounded-md hover:bg-red-600 flex items-center">
                                        <x-heroicon-s-trash class="w-4 h-4 mr-1" />
                                        Clear Chat
                                    </button>
                                    <button
                                        class="px-3 py-1.5 text-sm bg-green-500 text-white rounded-md hover:bg-green-600 flex items-center">
                            ...
                            <div class="flex-1 flex flex-col">
                                            ...
                                            @else
                                                <div class="flex justify-end">
                                                    <div class="max-w-xs lg:max-w-md">
                                                        ...
                                                        <p class="text-xs text-gray-400 text-right mt-1">{{ $message['timestamp'] }}
                                                        </p>
                                                    </div>
                                                </div>
                                            @endif
                                ...
                                <div
                                    ...
                                    <form wire:submit.prevent="sendMessage" class="flex items-center space-x-3">
                                        <input type="text" wire:model="currentMessage" placeholder="Type your message..."
                                    ...
                                    @else
                                        <div
                                            ...
                                            <div class="flex items-start space-x-2">
                                                <x-heroicon-s-document class="w-4 h-4 text-green-500 mt-0.5" />
                                                <div class="flex-1 min-w-0">
                                                    <p class="text-sm font-medium text-gray-900 dark:text-white">SmartFlow.docx
                                                    </p>
                                                    <p class="text-xs text-green-600 mt-1">Synced</p>
                                                </div>
                                            </div>
        ...
        @if($showDetailsModal && $detailsAssistantId)
            ...
            @if($details && $detailsAssistant)
                ...
                <div class="fixed inset-0 z-50 overflow-y-auto">
                    <div class="flex min-h-full items-center justify-center p-4">
                        <div class="relative transform overflow-hidden rounded-xl bg-white dark:bg-gray-800 text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-4xl"
                            wire:click.stop>
                            <!-- Header -->
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div
                                        ...
                                        <div class="flex items-center space-x-2 mb-4">
                                            ...
                                            <h4 class="text-sm font-semibold text-gray-900 dark:text-white">Assistant Overview
                                            </h4>
                                        </div>
                                        <div class="space-y-3">
                                            <div class="flex justify-between items-center">
                                                ...
                                                <span
                                                    class="text-sm font-medium text-gray-900 dark:text-white">{{ $details['total_documents'] }}</span>
                                            </div>
                                            <div class="flex justify-between items-center">
                                                <span class="text-sm text-gray-600 dark:text-gray-400">Sync Progress</span>
                                                <span
                                                    class="text-sm font-medium text-green-600">{{ $details['sync_progress'] }}%</span>
                                            </div>
                                            <div class="flex justify-between items-center">
                                                <span class="text-sm text-gray-600 dark:text-gray-400">Overall Status</span>
                                                <div class="flex items-center space-x-2">
                                                    <div
                                                        class="w-2 h-2 {{ $details['overall_status'] === 'Fully Synced' ? 'bg-green-500' : 'bg-yellow-500' }} rounded-full">
                                                    </div>
                                                    <span
                                                        class="text-sm font-medium {{ $details['overall_status'] === 'Fully Synced' ? 'text-green-600' : 'text-yellow-600' }}">{{ $details['overall_status'] }}</span>
                                                </div>
                                            </div>
                                            <div class="mt-4">
                                                <div class="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-2">
                                                    <div class="bg-green-500 h-2 rounded-full"
                                                        style="width: {{ $details['sync_progress'] }}%"></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div
                                        ...
                                        <div class="flex items-center space-x-2 mb-4">
                                            ...
                                            <h4 class="text-sm font-semibold text-gray-900 dark:text-white">OpenAI Resources
                                            </h4>
                                        </div>
                                        <div class="space-y-4">
                                            <!-- AI Assistant -->
                                            <div>
                                                <div class="flex items-center justify-between mb-2">
                                                    ...
                                                    <div class="flex items-center space-x-2">
                                                        <div
                                                            class="w-2 h-2 {{ $details['openai_assistant_id'] ? 'bg-green-500' : 'bg-gray-400' }} rounded-full">
                                                        </div>
                                                        <span
                                                            class="text-xs font-medium {{ $details['openai_assistant_id'] ? 'text-green-600' : 'text-gray-600' }}">
                                                            {{ $details['openai_assistant_id'] ? 'Created' : 'Not Created' }}
                                                        </span>
                                                    </div>
                                                </div>
                                                <p class="text-xs text-gray-500 dark:text-gray-400 mb-2">OpenAI Assistant
                                                    Instance</p>
                                            ...
                                            <div>
                                                <div class="flex items-center justify-between mb-2">
                                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Knowledge
                                                        Base</span>
                                                    <div class="flex items-center space-x-2">
                                                        <div
                                                            class="w-2 h-2 {{ $details['openai_vector_store_id'] ? 'bg-green-500' : 'bg-gray-400' }} rounded-full">
                                                        </div>
                                                        <span
                                                            class="text-xs font-medium {{ $details['openai_vector_store_id'] ? 'text-green-600' : 'text-gray-600' }}">
                                                            {{ $details['openai_vector_store_id'] ? 'Ready' : 'Pending' }}
                                                        </span>
                                                    </div>
                                                </div>
                                                <p class="text-xs text-gray-500 dark:text-gray-400 mb-2">Vector Store for
                                                    Document Search</p>
                                    ...
                                    <div
                                        ...
                                        <div class="flex items-center space-x-2 mb-4">
                                            ...
                                            <h4 class="text-sm font-semibold text-gray-900 dark:text-white">Document Status
                                                Summary</h4>
                                        </div>
                                        <div class="space-y-3">
                                            @if($details['total_documents'] > 0)
                                                <div class="flex items-center justify-between">
                                                    ...
                                                    <span
                                                        class="text-sm font-medium text-green-600 bg-green-50 dark:bg-green-900/20 px-2 py-1 rounded">
                                                        {{ $details['synced_documents'] }}
                                                        document{{ $details['synced_documents'] !== 1 ? 's' : '' }}
                                                    </span>
                                                </div>
                                                @if($details['synced_documents'] < $details['total_documents'])
                                                    <div class="flex items-center justify-between">
                                                        ...
                                                        <span
                                                            class="text-sm font-medium text-yellow-600 bg-yellow-50 dark:bg-yellow-900/20 px-2 py-1 rounded">
                                                            {{ $details['total_documents'] - $details['synced_documents'] }}
                                                            document{{ ($details['total_documents'] - $details['synced_documents']) !== 1 ? 's' : '' }}
                                                        </span>
                                                    </div>
                                                @endif
                                            @else
                                    ...
                                    <div
                                        ...
                                        <div class="grid grid-cols-2 gap-2">
                                            <button wire:click="editSpecificAssistant({{ $detailsAssistantId }})"
                                                class="flex items-center justify-center space-x-2 bg-purple-600 text-white px-3 py-2 rounded-lg text-xs font-medium hover:bg-purple-700 transition-colors">
                                                <x-heroicon-s-cog-6-tooth class="w-4 h-4" />
                                                <span>Manage</span>
                                            </button>
                                            <button wire:click="openChat({{ $detailsAssistantId }})"
                                                class="flex items-center justify-center space-x-2 bg-blue-600 text-white px-3 py-2 rounded-lg text-xs font-medium hover:bg-blue-700 transition-colors">
                                                <x-heroicon-s-chat-bubble-left-right class="w-4 h-4" />
                                                <span>Open Chat</span>
                                            </button>
                                            @if($details['openai_assistant_id'])
                                                <a href="https://platform.openai.com/assistants" target="_blank"
                                                    class="flex items-center justify-center space-x-2 bg-sky-500 text-white px-3 py-2 rounded-lg text-xs font-medium hover:bg-sky-600 transition-colors">
                                                    <x-heroicon-s-arrow-top-right-on-square class="w-4 h-4" />
                                                    <span>OpenAI Dashboard</span>
                                                </a>
                                            @else
                                                <button disabled
                                                    class="flex items-center justify-center space-x-2 bg-gray-400 text-white px-3 py-2 rounded-lg text-xs font-medium cursor-not-allowed">
                                                    <x-heroicon-s-arrow-top-right-on-square class="w-4 h-4" />
                                                    <span>Not Available</span>
                                                </button>
                                            @endif
                                            <button wire:click="syncAssistant({{ $detailsAssistantId }})"
                                                class="flex items-center justify-center space-x-2 bg-purple-600 text-white px-3 py-2 rounded-lg text-xs font-medium hover:bg-purple-700 transition-colors">
                            ...
                            <!-- Footer -->
                            <div
                                class="bg-gray-50 dark:bg-gray-700 px-6 py-4 border-t border-gray-200 dark:border-gray-600 flex justify-between items-center">
