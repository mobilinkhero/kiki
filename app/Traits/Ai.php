...
trait Ai
{
    ...
    public function personalAssistantResponse(
        ...
        try {
            ...
            $systemContext .= "\n\n🌍 LANGUAGE MATCHING RULE:\nALWAYS respond in the EXACT SAME language and format the user is using.\n\n📝 Examples:\n- User writes: 'main company register karna chahta hun' (Roman Urdu)\n  → You respond: 'Bilkul! Main aapki madad kar sakta hun...' (Roman Urdu)\n\n- User writes: 'میں کمپنی رجسٹر کرنا چاہتا ہوں' (Urdu script)\n  → You respond: 'بالکل! میں آپ کی مدد...' (Urdu script)\n\n- User writes: 'I want to register a company' (English)\n  → You respond: 'Sure! I can help you...' (English)\n\n⚠️ CRITICAL: Detect user's language style from their FIRST message and MAINTAIN it throughout the conversation. If they switch languages, switch with them.";

            // ✅ FEATURE: Human Handoff - Smart & Universal Protocol
    ...
    protected function useOpenAIAssistantsAPI(
        ...
        try {
            ...
            $runRequestData = [
                ...
                'additional_instructions' => ($assistant->allow_buttons ? "\nYou can attach up to 3 interactive buttons to your response using the format: {{BUTTON:Label}}. labels must be short (max 20 chars). Use buttons for choices, confirmations, or next steps. Example: {{BUTTON:Buy Basic}} {{BUTTON:More Info}}" : "") . "\n\n� SCOPE: ONLY answer questions about YOUR business services (defined in knowledge base). If user asks off-topic → Politely decline & redirect to your services.\n\n🌍 LANGUAGE: Match user's language/format exactly.\n\n🚨 HANDOFF: Use {{HANDOFF}} when: user sends file, asks for human, issue beyond capability, user abusive, high-stakes, or uncertain.",
            ];
